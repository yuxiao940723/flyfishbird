...
export default class DataDealer extends cc.Component {
    ...
    async dealData(dataParent: Object, nodeInLanguage: boolean, async: boolean, dealerKey: string, priority: number, taskTag: string, bundle: string) {
        if (this.dataParent) {
        ...
        if (async) {
            await ffb.taskDispatcher.addTaskToPriorityQueens(this.dispatchPriority, taskTag, this._dealData, this);
        } else {
            await this._dealData();
        }
    }
